...
class TestConfigManager(unittest.TestCase):
    ...
    def test_config_manager_initialization(self):
        ...
        self.assertEqual(len(manager.get_providers()), 0)
    ...
    def test_load_from_env_file(self):
        ...
        try:
            manager = ConfigManager(env_file_path=temp_env_path)
            manager.load_configuration()
            
            providers = manager.get_providers()
            self.assertEqual(len(providers), 1)
            self.assertEqual(providers[0].api_key, 'file-api-key')
            self.assertEqual(providers[0].base_url, 'https://file.example.com')
            self.assertEqual(providers[0].model, 'file-model')
    ...
    def test_get_primary_provider(self):
        ...
        self.assertEqual(primary.api_key, 'test-key')
    ...
    def test_get_provider_by_model(self):
        ...
        provider = manager.get_provider_by_model('test-model')
        self.assertIsNotNone(provider)
        self.assertEqual(provider.api_key, 'test-key')
    
    def test_has_valid_configuration(self):
        """测试检查是否有有效的配置"""
        os.environ['LLM_API_KEY'] = 'test-key'
        os.environ['LLM_BASE_URL'] = 'https://api.example.com'
        os.environ['LLM_MODEL'] = 'test-model'
        ...
        self.assertTrue(manager.has_valid_configuration())
    ...
    def test_get_configuration_status(self):
        ...
        self.assertEqual(status['is_loaded'], True)
        self.assertEqual(status['providers_count'], 1)
        self.assertEqual(status['has_valid_config'], True)
        self.assertEqual(status['primary_provider'], 'test-model')
    ...
    def test_reload_configuration(self):
        ...
        # 修改环境变量
        os.environ['LLM_API_KEY'] = 'new-key'
        ...
        primary = manager.get_primary_provider()
        self.assertEqual(primary.api_key, 'new-key')
